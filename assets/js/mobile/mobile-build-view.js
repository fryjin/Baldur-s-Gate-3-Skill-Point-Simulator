...
function renderSpells(build){
  ...
  const content=`<div class="m-spells-view">
    ...
    <section class="m-spell-controls" aria-label="法术搜索与筛选"><input id="m-spell-search" value="${esc(state.spellQuery)}" placeholder="搜索法术"><div class="m-filter-scroll">${filters.map(f=>`<button class="${String(f)===String(state.spellFilter)?'active':''}" data-m-filter="${f}">${f==='all'?'全部':f==='selected'?'已选':f==='prepared'?'已准备':f===0?'戏法':f+'环'}</button>`).join('')}</div></section>
