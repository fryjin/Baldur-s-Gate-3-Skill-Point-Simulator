...
assert len(report_equipment_keys)==130,f"expected 130 equipment keys, got {len(report_equipment_keys)}"
assert not unresolved,f"report still has unresolved rows: {[x.get('key') for x in unresolved]}"
assert report.get("parsed_rows",{}).get("spell")==189
assert report.get("parsed_rows",{}).get("equipment")==130
assert not(ROOT/"assets/hd/spells/BG3_WIKI_HD_ASSET_REPORT.json").exists(),"duplicate report still exists"
