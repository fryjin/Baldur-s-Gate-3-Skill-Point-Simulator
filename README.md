...
- 修复法术数量条与第一条法术重叠。
- 修复幻想森林背景被页面底色遮挡。
- 法术图标仅请求一次本地 PNG，不再逐个尝试多种扩展名和远程地址。
- 使用 IntersectionObserver 仅加载视口附近图标，并缓存本次会话中的失败路径。
- 子职业、专长、技能和职业能力点击后立即生效，不再要求右侧重复确认。
- 页面状态文案由“已确认”统一为“已完成”。

项目仍优先使用仓库已有图片：`assets/images/spells/<spellKey>.png`。
